...
def plot_main_metrics(data: Dict[str, np.ndarray], output_path: Path):
    """Create main metrics plot (WER, Bitrate).
    ...
    # 1. WER (Intelligibility)
    ax1 = fig.add_subplot(gs[0, 0])
    ax1.plot(x, data['wer'], 's-', color=colors['wer'], linewidth=2.5,
             markersize=7, markeredgecolor='white', markeredgewidth=1.5, label='WER')
    ax1.fill_between(x, data['wer'], alpha=0.2, color=colors['wer'])
    
    best_wer_idx = np.argmin(data['wer'])
    ax1.plot(x[best_wer_idx], data['wer'][best_wer_idx], 'o', 
             markersize=15, markerfacecolor='yellow', markeredgecolor=colors['wer'], 
             markeredgewidth=3, zorder=5)
    
    ax1.set_xlabel('Prosody Update Rate (Hz)', fontsize=11, fontweight='bold')
    ax1.set_ylabel('Word Error Rate', fontsize=11, fontweight='bold')
    ax1.set_title('Intelligibility (Lower is Better)', fontsize=12, fontweight='bold', pad=10)
    ...
    ax2.plot(x, data['bitrate_bps'], 'D-', color=colors['bitrate'], linewidth=2.5,
             markersize=7, markeredgecolor='white', markeredgewidth=1.5, label='Total Bitrate')
    
    ax2.set_xlabel('Prosody Update Rate (Hz)', fontsize=12, fontweight='bold')
    ax2.set_ylabel('Bitrate (bps)', fontsize=12, fontweight='bold')
    ax2.set_title('Bitrate Breakdown vs Prosody Rate', fontsize=14, fontweight='bold', pad=15)
...
def plot_nisqa_dimensions(data: Dict[str, np.ndarray], output_path: Path):
    ...
    for idx, (key, label, color, marker) in enumerate(dimensions):
        ...
        # Dynamic Y-axis limits to highlight distribution
        # y_range = y.max() - y.min()
        # margin = max(y_range * 0.2, 0.1)  # At least 0.1 margin
        # ax.set_ylim(y.min() - margin, y.max() + margin)
        
        # Absolute Y-axis limits starting from 0 as requested
        ax.set_ylim(3.5, 4.8)
...
def plot_perceptual_metrics(data: Dict[str, np.ndarray], output_path: Path):
    ...
    metrics = [
        ('nisqa_mos', 'NISQA MOS (Naturalness)', '#2ecc71', 'o'),
        ('pesq', 'PESQ (Perceptual Quality)', '#9b59b6', 's'),
        ('stoi', 'STOI (Intelligibility)', '#f39c12', '^'),
        ('speaker_similarity', 'Speaker Similarity', '#3498db', 'D'),
    ]
    
    for idx, (key, label, color, marker) in enumerate(metrics):
        ax = axes.flat[idx]
        y = data[key]
        ...
        # Absolute Y-axis limits starting from 0 as requested
        if key.startswith('nisqa'):
            ax.set_ylim(3.5, 4.8)
        elif key == 'pesq':
            ax.set_ylim(0, 3)
        elif key == 'stoi':
            ax.set_ylim(0, 0.5)
        elif key == 'speaker_similarity':
            ax.set_ylim(0.4, 0.8)
